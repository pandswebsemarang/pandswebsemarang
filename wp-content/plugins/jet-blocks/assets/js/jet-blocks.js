( function( $, madxartworkFrontend, madxartwork ) {
	...
	var JetBlocks = {
		...
		init: function() {
			...
			madxartworkFrontend.hooks.addAction( 'frontend/element_ready/section', JetBlocks.setStickySection );
			madxartworkFrontend.hooks.addAction( 'frontend/element_ready/container', JetBlocks.setStickySection );
		...
		hamburgerPanel: function( $scope ) {
			var $panel        = $( '.jet-hamburger-panel', $scope ),
				...
				timer,
				timer2,
				editMode      = Boolean( madxartworkFrontend.isEditMode() ),
				$html         = $( 'html' ),
				settings      = $panel.data( 'settings' ) || {},
				eContainer    = $scope.parents( '.e-container' );

			function fixmadxartworkContainerZIndex( e, open = true ) {
				if ( open ) {
				...
				$toggleButton.on( 'touchend', function( event ) {
					...
					if ( ! $panel.hasClass( 'open-state' ) ) {
						timer = setTimeout( function() {
							fixmadxartworkContainerZIndex( $( this ) );
						...
						timer2 = setTimeout( function() {
							fixmadxartworkContainerZIndex( $( this ), false );
						}, 400 );
					}
				} );
				...
				$.ajax( {
					...
					success: function( responce, textStatus, jqXHR ) {
						...
						Promise.all( JetBlocks.addedAssetsPromises ).then( function( value ) {
							...
							JetBlocks.madxartworkFrontendInit( $contentHolder );
		...
		stickySection: function() {
			var stickySection = {
				...
				isEditMode: Boolean( madxartworkFrontend.isEditMode() ),
		...
		setStickySection: function( $scope ) {
			var setStickySection = {
				...
				getSectionSetting: function( setting ){
					...
					if ( editMode ) {
						if ( ! madxartworkFrontend.hasOwnProperty( 'config' ) ) {
							return;
						}

						if ( ! madxartworkFrontend.config.hasOwnProperty( 'elements' ) ) {
							return;
						}

						if ( ! madxartworkFrontend.config.elements.hasOwnProperty( 'data' ) ) {
							return;
						}

						var modelCID = this.target.data( 'model-cid' ),
							editorSectionData = madxartworkFrontend.config.elements.data[ modelCID ];
		...
		isEditMode: function() {
			return Boolean( madxartworkFrontend.isEditMode() );
		},

		madxartworkFrontendInit: function( $container ) {
	...
	$( window ).on( 'madxartwork/frontend/init', JetBlocks.init );
...
}( jQuery, window.madxartworkFrontend, window.madxartwork, window.JetHamburgerPanelSettings ) );
