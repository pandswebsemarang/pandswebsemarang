( function ( $ ) {
	...
	var JetWooProductGallery = {
		...
		productGallerySlider: function ( $scope ) {
			...
			const
				...
				widgetSettings = JetWooProductGallery.getmadxartworkElementSettings( $scope );
			...
			if ( swiperContainer.find( '.jet-woo-product-gallery__image-item' ).length > 1 ) {
				...
				if ( swiperSettings.showPagination ) {
					if ( 'thumbnails' === swiperSettings.paginationType ) {
						...
						if ( window.madxartworkFrontend && ! $.isEmptyObject( widgetSettings ) ) {
							currentDeviceSlidePerView = +widgetSettings['pagination_thumbnails_columns'];

							if ( 'desktop' !== window.madxartworkFrontend.getCurrentDeviceMode() ) {
								currentDeviceSlidePerView = +widgetSettings[ 'pagination_thumbnails_columns_' + window.madxartworkFrontend.getCurrentDeviceMode() ];
							}
						} else {
		...
		handleSwiperBreakpoints: function ( widgetSettings, swiperParameters, paramsKeys ) {
			...
			const
				madxartworkBreakpoints = window.madxartworkFrontend.config.responsive.activeBreakpoints,
				madxartworkBreakpointsValues = madxartworkFrontend.breakpoints.getBreakpointValues(),
			...
			Object.keys( madxartworkBreakpoints ).reverse().forEach( breakpointName => {
				...
				swiperParameters.breakpoints[ madxartworkBreakpoints[ breakpointName ].value ] = {
			...
			Object.keys( swiperParameters.breakpoints ).forEach( breakpoint => {
				...
				if ( breakpointValue === madxartworkBreakpoints.mobile.value || breakpointValue + 1 === madxartworkBreakpoints.mobile.value ) {
					breakpointToUpdate = 0;
				} else if ( madxartworkBreakpoints.widescreen && ( breakpointValue === madxartworkBreakpoints.widescreen.value || breakpointValue + 1 === madxartworkBreakpoints.widescreen.value ) ) {
					breakpointToUpdate = breakpointValue;
				} else {
					const currentBreakpointIndex = madxartworkBreakpointsValues.findIndex( madxartworkBreakpoint => {
						return breakpointValue === madxartworkBreakpoint || breakpointValue + 1 === madxartworkBreakpoint;
					});

					breakpointToUpdate = madxartworkBreakpointsValues[ currentBreakpointIndex - 1 ];
				}
	...
	$( window ).on( 'madxartwork/frontend/init', JetWooProductGallery.init );
