( function( $ ) {
	...
	var JetEngine = {
		...
		widgetListingGrid: function( $scope ) {
			...
			if ( hasLazyLoad ) {
				...
				if ( window.madxartworkFrontend && window.madxartworkFrontend.isEditMode()
					...
					widgetSettings = JetEngine.getEditorElementSettings( $scope.closest( '.madxartwork-widget' ) );
			...
			if ( navSettings && navSettings.enabled ) {
				...
				switch ( loadMoreType ) {
					case 'click':
						...
						if ( navSettings.more_el ) {
							...
							if ( $button.length ) {
								...
								if ( page === pages && ! window.madxartwork ) {
					...
					case 'scroll':
						...
						if ( ( ! window.madxartworkFrontend || ! window.madxartworkFrontend.isEditMode() ) && ! $slider.length ) {
		...
		runMasonry: function( $masonry ) {
			...
			if ( $eWidget.length ) {
				var settings     = JetEngine.getmadxartworkElementSettings( $eWidget ),
					breakpoints  = {},
				...
				if ( $.isFunction( $.fn.imagesLoaded ) ) {
					$slider.imagesLoaded().always( function( instance ) {
						...
						if ( $slider.hasClass( 'slick-initialized' ) ) {
							...
							if ( $eWidget.length ) {
								var settings     = JetEngine.getmadxartworkElementSettings( $scope ),
									eBreakpoints = window.madxartworkFrontend.config.responsive.activeBreakpoints,
									responsive   = [];
		...
		updateAddedStyles: function() {
			if ( window.JetEngineSettings && window.JetEngineSettings.addedPostCSS ) {
				$.each( window.JetEngineSettings.addedPostCSS, function( ind, cssID ) {
					JetEngine.addedStyles.push( 'madxartwork-post-' + cssID );
	...
	$( window ).on( 'madxartwork/frontend/init', JetEngine.init );
