( function( $ ) {
	...
	var WidgetUAELGoogleMapHandler = function ( $scope, $ ) {
		...
		var selector                = $scope.find( '.uael-google-map' ).eq(0),
			...
			marker_cluster 			= [],
			device_size 			= madxartworkFrontend.getCurrentDeviceMode();
	...
	$( window ).on( 'madxartwork/frontend/init', function () {
		...
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-google-map.default', WidgetUAELGoogleMapHandler );
	});
} )( jQuery );
