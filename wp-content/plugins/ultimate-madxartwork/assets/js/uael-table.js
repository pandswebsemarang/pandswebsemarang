( function( $ ) {
	...
	var WidgetUAELTableHandler = function( $scope, $ ) {
		...
		//Search entries
		var search_entry = $( '.madxartwork-element-' + node_id + ' #' + uael_table_id[0].id ).data( 'searchable' );
		...
		//Show entries select
		var show_entry = $( '.madxartwork-element-' + node_id + ' #' + uael_table_id[0].id ).data( 'show-entry' );
		...
		//Sort entries
		var sort_table = $( '.madxartwork-element-' + node_id + ' #' + uael_table_id[0].id ).data( 'sort-table' );

		if ( 'yes' == sort_table ) {
			$( '.madxartwork-element-' + node_id + ' #' + uael_table_id[0].id + ' th' ).css({'cursor': 'pointer'});
	...
	$( window ).on( 'madxartwork/frontend/init', function () {
		...
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-table.default', WidgetUAELTableHandler );
	});
} )( jQuery );
