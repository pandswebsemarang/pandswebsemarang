( function( $ ) {
	...
	var WidgetUAELCafStylerHandler = function( $scope, $ ) {
		...
		if ( 'undefined' == typeof $scope )
			return;
		var	cafRadioFields = $scope.find('input[type=radio]');

		var	cafSelectFields = $scope.find('select');

		cafRadioFields.after( "<span class='uael-caf-radio-custom'></span>" );
	...
	var UAELFancyText = function() {
		...
		var nodeclass           = '.madxartwork-element-' + id;
			...
			if( 'custom' == trigger ) {
				if( 'yes' == overlay ) {
					...
					if( ! isElEditMode ) {
						...
						if( typeof madxartworkFrontend.waypoint !== 'undefined' ) {
							madxartworkFrontend.waypoint(
								$this,
								tourPlay,
								{
	...
	var WidgetUAELFancyTextHandler = function( $scope, $ ) {
		...
		var selector = $( '.madxartwork-element-' + node_id );
	...
	var WidgetUAELTimelineHandler = function( $scope, $ ) {
		...
		var $main_class			= $scope.find(".madxartwork-widget-uael-timeline");
		...
		if ( post_selector.hasClass( 'uael-timeline-infinite-load' ) ) {
			...
			post_selector.infinitescroll(
				{
	                navSelector     : '.madxartwork-element-' + node_id + ' .uael-timeline-pagination',
	                nextSelector    : '.madxartwork-element-' + node_id + ' .uael-timeline-pagination a.next',
	                itemSelector    : '.madxartwork-element-' + node_id + ' .uael-timeline-field',
	                prefill         : true,
	                bufferPx        : 200,
	                loading         : {
						msgText         : 'Loading',
	...
	$( window ).on( 'madxartwork/frontend/init', function () {
		...
		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-fancy-heading.default', WidgetUAELFancyTextHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-ba-slider.default', WidgetUAELBASliderHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-hotspot.default', WidgetUAELHotspotHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-timeline.default', WidgetUAELTimelineHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-content-toggle.default', WidgetUAELContentToggleHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-gf-styler.default', WidgetUAELGFStylerHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-cf7-styler.default', WidgetUAELCF7StylerHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-image-gallery.default', WidgetUAELImageGalleryHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-video.default', WidgetUAELVideoHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-video-gallery.default', WidgetUAELVideoGalleryHandler );

		madxartworkFrontend.hooks.addAction( 'frontend/element_ready/uael-caf-styler.default', WidgetUAELCafStylerHandler );

		if( isElEditMode ) {

			madxartwork.channels.data.on( 'element:after:duplicate element:after:remove', function( e, arg ) {
				$( '.madxartwork-widget-uael-ba-slider' ).each( function() {
