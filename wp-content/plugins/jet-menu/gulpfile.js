...
gulp.task( 'css-editor', () => {
	return gulp.src( './includes/madxartwork/assets/editor/scss/editor.scss' )
		.pipe( sass( { outputStyle: 'compressed' } ) )
		.pipe( autoprefixer( {
			browsers: ['last 10 versions'],
			cascade:  false
		...
		.pipe( gulp.dest( './includes/madxartwork/assets/editor/css/' ) )
		.pipe( notify( 'Compile Sass Done!' ) );
} );
...
gulp.task( 'watch', () => {
	...
	gulp.watch( './includes/madxartwork/assets/editor/scss/**', gulp.series( 'css-editor' ) );
} );
