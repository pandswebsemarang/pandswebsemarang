...
"use strict";
(self["webpackChunkmadxartwork_pro"] = self["webpackChunkmadxartwork_pro"] || []).push([["video-playlist"],{
...
// Copied from the core, original path: madxartwork/assets/dev/js/frontend/handlers/base-tabs.js.
class baseTabs extends madxartworkModules.frontend.handlers.Base {
  getDefaultSettings() {
    return {
      selectors: {
      ...
      autoExpand: true,
      keyDirection: {
        ArrowLeft: madxartworkFrontendConfig.is_rtl ? 1 : -1,
        ArrowUp: -1,
        ArrowRight: madxartworkFrontendConfig.is_rtl ? -1 : 1,
        ArrowDown: 1
      }
    };
  }
  ...
  activateTab(tabIndex) {
    ...
    $requestedContent[settings.showTabFn](animationDuration, () => madxartworkFrontend.elements.$window.trigger('resize'));
...
async function triggerEvent(widgetObject, eventType, eventTrigger, positionInVideoList) {
  ...
  jQuery('body').trigger('madxartwork-video-playList', currentEvent);
}
...
class VideoPlaylistHandler extends _baseTabs.default {
  ...
  initEditorListeners() {
    ...
    this.editorListeners.push({
      event: 'madxartworkPlaylistWidget:fetchVideoData',
      to: madxartwork.channels.editor,
      callback: e => {
        this.getCurrentPlayerSelected().setVideoProviderData().then(() => {
          e.currentItem = this.getCurrentItemSelected();
          madxartwork.channels.editor.trigger('madxartworkPlaylistWidget:setVideoData', e);
        });
      }
    });
  }
  ...
  onInit() {
    ...
    const storageObject = madxartworkFrontend.storage.get(this.storageKey);
  ...
  handleFirstVideoActivation(isLazyLoad) {
    ...
    const playlistElement = document.querySelector('.madxartwork-element-' + this.playlistId + ' .e-tabs-main-area'),
          observer = madxartworkModules.utils.Scroll.scrollObserver({
      callback: event => {
      ...
      playListItem.playerInstance.create().then(() => {
        ...
        playListItem.playerInstance.handleEnded(() => {
          ...
          if (!this.watchedVideosArray.includes(endedVideoId) && this.watchedIndication) {
            ...
            madxartworkFrontend.storage.set(this.storageKey, JSON.stringify(this.watchedVideosArray));
