...
"use strict";
(self["webpackChunkmadxartwork_pro"] = self["webpackChunkmadxartwork_pro"] || []).push([["page-transitions-editor"],{
...
class Animate extends $e.modules.CommandBase {
   ...
   */
  apply() {
    const pageTransition = madxartwork.$previewContents[0].querySelector('e-page-transition');
...
class AnimatePageTransition extends $e.modules.hookData.After {
  ...
  prefix = 'settings_page_transitions_'; // Controls that the hook should listen to.
...
class PageTransitionPreview {
  ...
  run(component, route) {
    if ('panel/global/settings-page-transitions' === route) {
      (0, _utils.renderPageTransition)(madxartwork.documents.getCurrent().container);
  ...
  togglePageTransitionPreview() {
    ...
    const className = 'e-page-transition--preview',
          pageTransition = madxartwork.$previewContents[0].body.querySelector('e-page-transition');
...
function renderPageTransition(container) {
  let pageTransition = madxartwork.$previewContents[0].querySelector('e-page-transition');
  ...
  if (!pageTransition) {
    ...
    madxartwork.$previewContents[0].body.append(pageTransition);
  } // Disable the Page Transition if needed.


  pageTransition.toggleAttribute('disabled', !shouldRender);
  const settings = getPageTransitionSettings(container); // Iterate over the settings and set them as attributes.
  ...
  Object.entries(settings).forEach(_ref3 => {
    ...
    Object.entries(value).forEach(_ref4 => {
      ...
      let newKey = key; // Append the sub key only if it's not `value` (e.g. `url`), in order to avoid weird
...
class _default extends madxartworkModules.editor.utils.Module {
  /**
  ...
  bindEvents() {
    // Make sure that `window.madxartwork` is initialized.
    ...
    jQuery(window).on('madxartwork:init', () => this.onAnimateButtonClick());
  }
  /**
  ...
  onAnimateButtonClick() {
    madxartwork.channels.editor.on('madxartworkPageTransitions:animate', () => {
      $e.run('page-transitions/animate');
    });
  }
...
//# sourceMappingURL=page-transitions-editor.d1b925262ac181e8d57c.bundle.js.map