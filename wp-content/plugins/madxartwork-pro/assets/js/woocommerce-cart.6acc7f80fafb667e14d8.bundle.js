...
"use strict";
(self["webpackChunkmadxartwork_pro"] = self["webpackChunkmadxartwork_pro"] || []).push([["woocommerce-cart"],{
...
class Cart extends _base.default {
  ...
  bindEvents() {
    super.bindEvents();
    const selectors = this.getSettings('selectors');
    madxartworkFrontend.elements.$body.on('wc_fragments_refreshed', () => this.applyButtonsHoverAnimation());

    if ('yes' === this.getElementSettings('update_cart_automatically')) {
      this.$element.on('input', selectors.quantityInput, () => this.updateCart());
    }

    madxartworkFrontend.elements.$body.on('wc_fragments_loaded wc_fragments_refreshed', () => {
    ...
    madxartworkFrontend.elements.$body.on('added_to_cart', function (e, data) {
  ...
  onElementChange(propertyName) {
    ...
    if ('additional_template_select' === propertyName) {
      madxartworkPro.modules.woocommerce.onTemplateIdChange('additional_template_select');
    }
  }
  ...
  applyButtonsHoverAnimation() {
    ...
    if (elementSettings.checkout_button_hover_animation) {
      ...
      jQuery('.checkout-button').addClass('madxartwork-animation-' + elementSettings.checkout_button_hover_animation);
    }

    if (elementSettings.forms_buttons_hover_animation) {
      // This element is recaptured every time because the cart markup can refresh
      jQuery('.shop_table .button').addClass('madxartwork-animation-' + elementSettings.forms_buttons_hover_animation);
    }
  }
  /**
  ...
  hideHiddenInputsParentElements() {
    if (this.isEdit) {
      if (this.elements.$hiddenInput) {
        this.elements.$hiddenInput.parent('.form-row').addClass('madxartwork-hidden');
      }
    }
  }
...
//# sourceMappingURL=woocommerce-cart.6acc7f80fafb667e14d8.bundle.js.map