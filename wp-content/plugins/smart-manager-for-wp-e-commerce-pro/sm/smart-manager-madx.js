...
//                    SM.editor_state = Ext.decode(myJsonObj[SM.dashboard_state]);
	...
	var madxShowPrintButton = function(){
				...
				listeners: { 
					click: function () {
		                ...
		                Ext.DomHelper.append(Ext.getBody(), { 
		                    ...
		                    // src: jsonURL+'?cmd=exportCsvmadx&incVariation='+SM.incVariation+'&searchText='+SM.searchTextField.getValue()+'&fromDate='+fromDateTxt.getValue()+'&toDate='+toDateTxt.getValue()+'&active_module='+SM.activeModule+'&SM_IS_madx16='+SM_IS_madx16+''
		                    // src: ajaxurl + '?action=sm_include_file&file='+jsonURL+'&func_nm=exportCsvmadx&incVariation='+SM.incVariation+'&searchText='+SM.searchTextField.getValue()+'&fromDate='+fromDateTxt.getValue()+'&toDate='+toDateTxt.getValue()+'&active_module='+SM.activeModule+'&SM_IS_madx16='+SM_IS_madx16+''
		                    src: fileurl + '&file='+jsonURL+'&func_nm=exportCsvmadx&incVariation='+SM.incVariation+'&search_query[]='+encodeURIComponent(search_query)+'&search=advanced_search&searchText='+SM.searchTextField.getValue()+'&fromDate='+fromDateTxt.getValue()+'&toDate='+toDateTxt.getValue()+'&active_module='+SM.activeModule+'&SM_IS_madx16='+SM_IS_madx16+'&SM_IS_madx21='+SM_IS_madx21+'&SM_IS_madx22='+SM_IS_madx22+'&SM_IS_madx30='+SM_IS_madx30+'&security='+SM_NONCE,
		                }); 
					}
				}
			});
			...
			select: function () {
				pagingToolbar.emptyMsg = this.getValue() + ' ' + getText('list is empty');
                                ...
                                if(this.value == 'Products') {
                                      editorGrid.stateId = this.value.toLowerCase()+'EditorGridPanelmadx';
                                }
...
// for madx2.2	
if (SM_IS_madx22 == 'true' || SM_IS_madx30 == 'true') {
...
var batchUpdatePanel = new Ext.Panel({
            ...
            ,'->',
	{
		...
		listeners: { click: function () {
			...
			batchUpdateRecords(batchUpdatePanel,toolbarCount,cnt_array,store,jsonURL,batchUpdateWindow,radioValue,flag,pagingToolbar,products_search_flag,batch_limit,SM_IS_madx16,SM_IS_madx21,SM_IS_madx22,SM_IS_madx30);
		}}
	}]
});
...
var billingDetailsIframe = function(recordId){
	var billingDetailsWindow = new Ext.Window({
		stateId : 'billingDetailsWindowmadx',
...
// ======= orders ======
	...
	stateId : 'productsEditorGridPanelmadx',
				...
				// Show WPeC's product edit page in a Ext window instance.
					...
					if(columnIndex == editLinkColumnIndex) {
						var productsDetailsWindow = new Ext.Window({
							stateId : 'productsDetailsWindowmadx',
							collapsible:true,
							shadow : true,
							shadowOffset: 10,
							stateEvents : ['show','bodyresize','maximize'],
