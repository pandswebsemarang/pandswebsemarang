...
var batchUpdateRecords = function(batchUpdatePanel,toolbarCount,cnt_array,store,jsonURL,batchUpdateWindow,radioValue,flag,pagingToolbar,products_search_flag,sm_batch_limit,SM_IS_madx16,SM_IS_madx21,SM_IS_madx22,SM_IS_madx30)
{	
	var sb;
	var selectedField     = [];
	var selectedFields    = [];
	...
	if (wpscRunning == 1) {
		...
		var firstTextAreaCmp         = batchUpdatePanel.items.items[0].items.items[0].items.items[15];
	} else {
...
//							dropDown3Value[sb] = (firstCategoryDropdownCmp.value)? firstCategoryDropdownCmp.value : batchUpdatePanel.items.items[sb].items.items[0].items.items[4].value;
						...
						if(wpscRunning == 1){ //code for order notes for wpec
							if (firstTextAreaCmp.getValue()) {
								value[sb] = firstTextAreaCmp.getValue();
							}
						}
						...
						if ( wpscRunning == 1 ) {	
							...
							if(batchUpdatePanel.items.items[sb].items.items[15] != undefined)
								var textFieldValue = (batchUpdatePanel.items.items[sb].items.items[15].getValue()) ? batchUpdatePanel.items.items[sb].items.items[15].getValue() : '';
...
//		batchUpdateWindow.loadMask.show();
        ...
        function batch_update(count,total_records) {
            ...
            var arr = new Array();
            updatecnt = 0;
            if (total_records > sm_batch_limit) {
                fupdatecnt = sm_batch_limit;
            }
            else{
            ...
            var task = new Ext.util.DelayedTask(function(){
                ...
                batchUpdateWindow.loadMask.show();
                if (madxRunning == 1 && (SM.activeModule == 'Products')) {
                    Ext.Ajax.request(o1);
                }
                else {
                    store.commitChanges();
//                    store.load();
            ...
            params: {
                    ...
                    if ((fupdatecnt+sm_batch_limit) <= total_records) {
                          fupdatecnt = fupdatecnt +sm_batch_limit;
                    }
                     else{
                        fupdatecnt = total_records;
                    }
...
var addProduct  = function(store,cnt_array,cnt,newCatName){
	...
	if (madxRunning == 1) {
		...
		SM.productsCols.regularPrice.colName +":'',"+  
