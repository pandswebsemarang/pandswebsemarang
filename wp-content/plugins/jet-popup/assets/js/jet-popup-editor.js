( function( $ ) {
	...
	JetPopupControlsViews = {
		...
		init: function() {
			...
			self.JetSearchView = window.madxartwork.modules.controls.BaseData.extend( {
							...
							url: function() {
								...
								if ( queryParams.length > 0 ) {
									$.each( queryParams, function( index, param ) {
										...
										if ( window.madxartwork.settings.page.model.attributes[ param ] ) {
											query += '&' + param + '=' + window.madxartwork.settings.page.model.attributes[ param ];
										}
									});
								}
			...
			window.madxartwork.addControlView( 'jet_popup_search', self.JetSearchView );
	...
	JetPopupEditor = {
		init: function() {
			let panel = jQuery( '#madxartwork-editor-wrapper' );
			...
			madxartwork.settings.page.model.on( 'change', JetPopupEditor.onPageSettingsChange );

			JetPopupControlsViews.init();
		},

		onPageSettingsChange: function( model ) {
			var iframe   = document.getElementById( 'madxartwork-preview-iframe' ),
			...
			if ( model.changed.hasOwnProperty( 'selected_close_button_icon' ) ) {
				...
				wp.apiFetch( {
					method: 'post',
					path: window.jetPopupData.getmadxartworkIconHtml,
					data: {
						iconData: iconData
					},
				} ).then( ( response ) => {
	...
	$( window ).on( 'madxartwork:init', JetPopupEditor.init );
