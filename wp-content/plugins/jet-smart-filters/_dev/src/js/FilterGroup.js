...
export default class FilterGroup {
	...
	ajaxRequestCompleted(response) {
		...
		if (this.provider) {
			this.$provider
				.closest('.madxartwork-widget-jet-engine-maps-listing,.jet-map-listing-block')
	...
	renderResult(result) {
		...
		// trigger madxartwork widgets
		if (window.madxartworkFrontend) {
			switch (this.provider) {
				case 'jet-engine':
					window.madxartworkFrontend.hooks.doAction('frontend/element_ready/jet-listing-grid.default', this.$provider, $);
					break;

				case 'epro-portfolio':
					window.madxartworkFrontend.hooks.doAction('frontend/element_ready/portfolio.default', this.$provider, $);
					break;
			}
