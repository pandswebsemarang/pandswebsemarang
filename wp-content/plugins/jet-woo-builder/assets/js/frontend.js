( function( $, madxartworkFrontend ) {
	...
	let JetWooBuilder = {
		...
		init: function() {
			...
			madxartworkFrontend.hooks.addFilter( 'jet-popup/widget-extensions/popup-data', JetWooBuilder.prepareJetPopup );
		...
		setAddressFieldsRequiredValidation: function ( wrapper ) {
			...
			let $widget = wrapper.closest( '.madxartwork-element' ),
				settings = JetWooBuilder.getmadxartworkElementSettings( $widget );
		...
		widgetProductsLoop: function( $scope ) {
			...
			let settings = JetWooBuilder.getmadxartworkElementSettings( $scope );
			...
			if ( settings && settings.switcher_enable ) {
				...
				$switcherControl.on( 'click.JetWooBuilder', function( event ) {
					...
					$.ajax( {
						...
						JetWooBuilder.madxartworkFrontendInit( $productsWrapper );
		...
		handleInputQuantityValue: function( $scope ) {
			...
			let $eWidget = $scope.closest( '.madxartwork-widget' ),
				settings = JetWooBuilder.getmadxartworkElementSettings( $eWidget );
		...
		prepareJetPopup: function( popupData, widgetData, $scope, event ) {
			...
			if ( widgetData['is-jet-woo-builder'] ) {
				...
				if ( $scope.hasClass( 'madxartwork-widget-jet-woo-products' ) || $scope.hasClass( 'madxartwork-widget-jet-woo-products-list' ) ) {
		...
		initCarousel: function( $target, options ) {
			...
			let $eWidget = $target.closest( '.madxartwork-widget' ),
			...
			let currentDeviceSlidePerView = +settings[ 'columns_' + madxartworkFrontend.getCurrentDeviceMode() ] || +settings['columns'];
		...
		madxartworkFrontendInit: function( $content ) {
	...
	$( window ).on( 'madxartwork/frontend/init', JetWooBuilder.init );
