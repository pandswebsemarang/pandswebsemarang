(function( $ ) {
	...
	window.JetSM = {
		...
		init: function() {
			...
			window.madxartwork.hooks.addFilter( 'elements/widget/contextMenuGroups', function( groups, element ) {
			...
			window.madxartwork.on( 'preview:loaded', function() {
				...
				var $previewBody = window.madxartwork.$preview.contents();
		...
		removeSkinClass: function( $el ) {
			...
			classes.forEach( function( className ) {
				if ( className.includes( 'madxartwork-element-skin-' ) ) {
		...
		resetSkin: function() {
			...
			window.madxartwork.saver.setFlagEditorChange( true );
		...
		getModal: function() {
			...
			if ( ! this.modal ) {
				...
				this.modal = window.madxartwork.dialogsManager.createWidget( 'lightbox', {
					id: 'jet-sm-skins-modal',
					closeButton: true
				} );

				this.modal.setHeaderMessage( '<div class="jet-sm-popup-close madxartwork-templates-modal__header__close madxartwork-templates-modal__header__close--normal madxartwork-templates-modal__header__item"><i class="eicon-close" aria-hidden="true" title="Close"></i><span class="madxartwork-screen-only">Close</span></div>' );
			}
		...
		applySkin: function() {
			...
			$.ajax({
				...
				if ( response.success ) {
					...
					window.madxartwork.$preview.contents().find( 'head' ).append( response.data.css );
					window.madxartwork.saver.setFlagEditorChange( true );
		...
		getModalMessage: function( skins ) {
			...
			if ( skins.length ) {
				...
				skins.forEach( function( skin ) {
					modalMessage += '<tr><td>' + skin.skin + '</td><td><button type="button" class="madxartwork-button madxartwork-button-success jet-sm-apply-skin" data-skin="' + skin.skin + '">Apply</button><a href="#" class="jet-sm-delete-skin" data-skin="' + skin.skin + '">Delete</a></td></tr>';
				});
	...
	$( window ).on( 'madxartwork:init', JetSM.init );
