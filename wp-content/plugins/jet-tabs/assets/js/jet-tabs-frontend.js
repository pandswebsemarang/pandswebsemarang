...
( function( $, madxartwork, settings ) {
	...
	var JetTabs = {
		...
		tabsInit: function( $scope ) {
			var $target         = $( '.jet-tabs', $scope ).first(),
				...
				settings        = $.extend( $target.data( 'settings' ) || {}, JetTabs.getmadxartworkElementSettings( $scope ) ),
				anchorSelectors = [],
				...
				$.ajax( {
					...
					success: function( responce, textStatus, jqXHR ) {
						...
						Promise.all( JetTabs.addedAssetsPromises ).then( function( value ) {
							...
							JetTabs.madxartworkFrontendInit( $contentHolder );
				...
				$.ajax( {
					...
					success: function( responce, textStatus, jqXHR ) {
						...
						Promise.all( JetTabs.addedAssetsPromises ).then( function( value ) {
							...
							JetTabs.madxartworkFrontendInit( $contentHolderInner );
	...
	$( window ).on( 'madxartwork/frontend/init', JetTabs.init );

	window.JetTabs = JetTabs;

}( jQuery, window.madxartworkFrontend, window.JetTabsSettings ) );
