jQuery( window ).on( 'madxartwork:init', function() {
	...
	madxartwork.hooks.addAction( 'panel/open_editor/widget/ee-offcanvas', function( panel, model, view ) {

		if ( 'ee-offcanvas.classic' !== view.$el.data('element_type') )
			return;

		madxartwork.channels.editor.off('ee:editor:offcanvas:open').on('ee:editor:offcanvas:open', function( event ) {

			var $element 		= view.$el,
				$element_id 	= $element.data('id'),
				$slidebar 		= window.madxartwork.$previewContents.find( '#ee-offcanvas-' + $element_id ),
				ocController 	= window.madxartworkFrontend.eeOffcanvas.controller;
	...
	var ControlSnazzy = madxartwork.modules.controls.BaseData.extend( {
		...
		getSnazzyDefaultOptions: function getSnazzyDefaultOptions() {
			...
			return {
				...
				dir 				: madxartwork.config.is_rtl ? 'rtl' : 'ltr',
				ajax 				: {
	...
	var ControlQuery = madxartwork.modules.controls.Select2.extend( {
			...
			return jQuery.extend( madxartwork.modules.controls.Select2.prototype.getSelect2DefaultOptions.apply( this, arguments ), {
				ajax: {
					transport: function transport( params, success, failure ) {
						...
						return madxartworkCommon.ajax.addRequest('ee_query_control_filter_autocomplete', {
							data 	: data,
							success : success,
							error 	: failure,
						});
		...
		addSpinner: function addSpinner() {
			...
			this.$el.find('.madxartwork-control-title').after('<span class="madxartwork-control-spinner ee-control-spinner">&nbsp;<i class="fa fa-spinner fa-spin"></i>&nbsp;</span>');
		},
	...
	madxartwork.addControlView( 'ee-query', ControlQuery );
	madxartwork.addControlView( 'ee-snazzy', ControlSnazzy );
} );