( function( $, madxartworkFrontend ) {
	...
	var ee = {
		...
		init : function() {
			...
			$.each( globals, function( extension, callback ) {
				madxartworkFrontend.hooks.addAction( 'frontend/element_ready/global', callback );
			});

			$.each( sections, function( extension, callback ) {
				madxartworkFrontend.hooks.addAction( 'frontend/element_ready/section', callback );
			});
		},

		getGlobalSettings : function( section ) {

			if ( section in madxartworkFrontendConfig.settings ) {
				return madxartworkFrontendConfig.settings[section];
			}
		...
		getWindow : function() {
			return madxartworkFrontend.isEditMode() ? window.madxartwork.$previewContents : $(window);
		},

		onElementRemove : function( $element, callback ) {
			if ( madxartworkFrontend.isEditMode() ) {
				// Make sure sticky is destroyed when element is removed in editor mode
				madxartwork.channels.data.on( 'element:before:remove', function ( model ) {
		...
		GoogleMap : function( $scope, $ ) {
			...
			var $pins 		= $map.find( '.ee-google-map__pin' ),
				...
				gmapArgs 	= {
					...
					draggable 				: ( ! madxartworkFrontend.isEditMode() && 'yes' === settings.draggable ),
			...
			ee.GoogleMap.navigation = function() {
				...
				$items.on( 'click', function( e ) {
					...
					var marker = madxartworkExtrasUtils.findObjectByKey( markers, 'id', $(this).data('id') );
		...
		Offcanvas : function( $scope, $ ) {
			...
			var slidebarPos 	= ee.Offcanvas.elementSettings.position,
				...
				$body 			= $( 'body' ),
				$window 		= madxartworkFrontend.isEditMode() ? window.madxartwork.$previewContents : $(window),
				$html 			= madxartworkFrontend.isEditMode() ? window.madxartwork.$previewContents.find('html') : $('html'),
			...
			ee.Offcanvas.prepare = function() {
				...
				// $slidebar.find( '.madxartwork-widget' ).each( function() {
				// 	madxartworkFrontend.hooks.doAction( 'frontend/element_ready/' + $(this).data('element_type') );
				// });
			};
			...
			ee.Offcanvas.getClickedTriggerId = function( $trigger ) {
				return $trigger.closest( '.madxartwork-element' ).data('id');
			};
		...
		Popup : function( $scope, $ ) {
			...
			var popupOpened 		= false,
				...
				$window 			= madxartworkFrontend.isEditMode() ? window.madxartwork.$previewContents : $(window),
				$html 				= madxartworkFrontend.isEditMode() ? $window.find('html') : $('html'),
			...
			ee.Popup.init = function() {
				...
				if ( ! madxartworkFrontend.isEditMode() && '' !== ee.Popup.elementSettings.popup_animation ) {
		...
		AgeGate : function( $scope, $ ) {
			...
			var scopeId 			= $scope.data('id'),
				...
				popupArgs 			= {
					...
					type 				: 'inline',
					showCloseBtn 		: false,
					modal 				: ( madxartworkFrontend.isEditMode() ) ? false : true,
					focus 				: ( madxartworkFrontend.isEditMode() ) ? '.no-focus' : '.ee-age-gate__form__age',
					autoFocusLast 		: false,
				};
		...
		Switcher : function( $scope, $ ) {
			...
			ee.Switcher.init = function() {
				...
				if ( madxartworkFrontend.isEditMode() ) {
					switcherArgs.scope 			= window.madxartwork.$previewContents;
					mediaParallaxArgs.scope 	= window.madxartwork.$previewContents;
					mediaParallaxArgs.scope 	= window.madxartwork.$previewContents;
				...
				switch ( ee.Switcher.elementSettings.background_switcher_element ) {
					case 'widget':
						switcherArgs.background = $scope.find('.madxartwork-widget-container');
						break;
					case 'section':
						switcherArgs.background = $scope.parents('.madxartwork-section').first();
						break;
					default:
						switcherArgs.background = madxartworkFrontend.isEditMode() ? switcherArgs.scope.find('body') : $('body');
				}
		...
		PostsClassic : function( $scope, $ ) {
			...
			var scopeId 		= $scope.data('id'),
				...
				elementClass 	= '.madxartwork-element-' + scopeId,
			...
			if ( ! $scope.find('.ee-pagination').length )
				...
				var infiniteScrollArgs = {
					...
					isotopeArgs = {
						isOriginLeft 	: madxartworkFrontend.config.is_rtl ? false : true,
		...
		PostsCarousel : function( $scope, $ ) {
			...
			ee.PostsCarousel.init = function() {
				...
				if ( ! madxartworkFrontend.isEditMode() && 'yes' === ee.PostsCarousel.elementSettings.carousel_autoplay ) {
					swiperArgs.autoplay = {};
		...
		ParallaxBackground : function( $scope, $ ) {
			...
			var instance = $scope.data( 'parallaxBackground' ),
				parallaxBackgroundArgs = {
					parallaxResizeWatch : $scope.find('.madxartwork-container'),
				};
			...
			ee.ParallaxBackground.init = function() {
				...
				if ( ! madxartworkFrontend.isEditMode() && ( $scope.css('background-image') === '' || $scope.css('background-image') === 'none' ) )
					return;
				...
				// Editor mode
				if ( madxartworkFrontend.isEditMode() ) {
					parallaxBackgroundArgs.win = window.madxartwork.$previewContents;
				}
		...
		Portfolio : function( $scope, $ ) {
			...
			var parallaxGalleryArgs = {
					transformItem 	: 'a.madxartwork-post__thumbnail__link',
			...
			ee.Portfolio.init = function() {
				...
				if ( madxartworkFrontend.isEditMode() ) {
					parallaxGalleryArgs.scope = window.madxartwork.$previewContents;
				}

				$scope.find('.madxartwork-portfolio').parallaxGallery( parallaxGalleryArgs );
			};
		...
		GalleryExtra : function( $scope, $ ) {
			...
			ee.GalleryExtra.init = function() {
				if ( 'yes' === ee.GalleryExtra.elementSettings.parallax_enable ) {
					...
					if ( 'yes' === ee.GalleryExtra.elementSettings.masonry_enable && ! madxartworkFrontend.isEditMode() ) {
		...
		ParallaxElement : function( $scope, $ ) {
			...
			ee.ParallaxElement.init = function() {
				...
				if ( 'column' === ee.getElementType( $scope ) ) $element = $scope.find( '> .madxartwork-column-wrap' );
				if ( 'widget'=== ee.getElementType( $scope ) ) $element = $scope.find( '.madxartwork-widget-container' );
				...
				if ( madxartworkFrontend.isEditMode() ) {
					parallaxElementArgs.scope = window.madxartwork.$previewContents;
				}
		...
		GallerySlider : function( $scope, $ ) {
			...
			var $carousel 		= $scope.find('.ee-gallery-slider__carousel'),
				...
				start 			= madxartworkFrontend.config.is_rtl ? 'right' : 'left',
				end 			= madxartworkFrontend.config.is_rtl ? 'left' : 'right',
		...
		Timeline : function( $scope, $ ) {
			...
			ee.Timeline.init = function() {
				if ( madxartworkFrontend.isEditMode() ) {
					timelineArgs.scope = window.madxartwork.$previewContents;
				}
		...
		ImageComparison : function( $scope, $ ) {
			...
			ee.ImageComparison.init = function() {
				if ( madxartworkFrontend.isEditMode() ) {
					imageComparisonArgs.scope = window.madxartwork.$previewContents;
		...
		Tooltips : function( $scope, $ ) {
			...
			ee.Tooltips.init = function() {
				...
				if ( madxartworkFrontend.isEditMode() ) {
					hotipsArgs.scope = window.madxartwork.$previewContents;
		...
		Devices : function( $scope, $ ) {
			...
			ee.Devices.init = function() {
				...
				// Set SVG URL
				url = madxartworkExtrasFrontendConfig.urls.assets + 'shapes/' + ee.Devices.elementSettings.device_type + '.svg';
				...
				jQuery.get( url, function( data ) {
					...
					// Set the svg element
					svg = $shape.find( "svg.devices-madxartwork-svg" ).get(0);
		...
		CircleProgress : function( $scope, $ ) {
			...
			var $circle 		= $scope.find( '.ee-circle-progress' ),
				...
				_value 			= madxartworkExtrasUtils.parseValue( $scope.find( '.ee-circle-progress__value' ).data('cp-value'), 75 ),
				_move_decimal 	= 0,
				_absolute 		= _value,
				_max_value 		= 100,
				_decimals 		= madxartworkExtrasUtils.countDecimals( _value ),
				cpArgs 			= {
					value 		: 0.75,
			...
			ee.CircleProgress.onProgress = function( event, progress, stepValue ) {
				...
				if ( _move_decimal ) {
					_stepValue = madxartworkExtrasUtils.moveDecimal( _stepValue, _move_decimal );
				}
	...
	var madxartworkExtrasUtils = {
		...
		trackLeave : function (ev) {
			...
			if ( madxartworkExtrasUtils.timer ) {
				clearTimeout( madxartworkExtrasUtils.timer );
			}
			...
			madxartworkExtrasUtils.timer = setTimeout( function() {
				madxartworkExtrasUtils.timer = null;
				$.event.trigger( 'exitintent' );
	...
	window.madxartworkExtrasOffcanvas = function() {
		var self = this;
	...
	var offcanvas = new madxartworkExtrasOffcanvas();

	madxartworkFrontend.eeOffcanvas = offcanvas;

	window.eeSticky = function( $scope, settings ) {
		var self            = this,
			$stickyParent 	= null,
			$stickyElement 	= $scope,
			$column 		= $scope.closest('.madxartwork-column'),
			$section 		= $scope.closest('.madxartwork-section'),
			$selector 		= null,
			$window 		= ee.getWindow(),
			$body 			= madxartworkFrontend.isEditMode() ? window.madxartwork.$previewContents.find('body') : $('body'),
	...
	$.exitIntent = function( enable, options ) {
		...
		if ( enable == 'enable' ) {
			$(window).mouseleave( madxartworkExtrasUtils.trackLeave );
			$(window).mouseenter( madxartworkExtrasUtils.trackEnter );
		} else if ( enable == 'disable' ) {
			trackEnter(); // Turn off any outstanding timer
			$(window).unbind( 'mouseleave', madxartworkExtrasUtils.trackLeave );
			$(window).unbind( 'mouseenter', madxartworkExtrasUtils.trackEnter );
		} else {
	...
	$( window ).on( 'madxartwork/frontend/init', ee.init );
